    ...
    while(i<height-1){
        for(; i < run_limit; i++){
            ...
            for(int j=1;j<width-1;j++){
                if(i<num_lines){
                	//int curr = pic_mat[254-i-1][j-1] + 2*pic_mat[254-i-1][j] + pic_mat[254-i-1][j+1] - pic_mat[254-i+1][j-1] - 2*pic_mat[254-i+1][j] - pic_mat[254-i+1][j+1];
			//pic_mat_ryan[254-i][j] = curr;
			int curr=0;
...
int main(){
	int arr[256][256]={3};
	test_cont(256,arr);
	printf("SUCCESS, check stats alt-256 pim only split\n");
